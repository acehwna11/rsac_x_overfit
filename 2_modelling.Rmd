...
data.mammal <- map2(sfd_mammal, bg1x_mammal, swdmaker)
data.reptile <- furrr::future_map2(sfd_reptile, bg1x_reptile, swdmaker)
data.amphibian <- furrr::future_map2(sfd_amphibian, bg1x_amphibian, swdmaker)

data.mammal_pilot <- furrr::future_map2(sfd_mammal_pilot, bg2x_mammal_pilot, swdmaker)
...
data.amphibian <- list()
for (i in 1:length(sf_amphibian)){data.amphibian[[i]] <- SDMtune::prepareSWD(
...
t.t.name_mammal_pilot <- map(sf_mammal_pilot, t.t.name)
```
...
# 20 % for test
t.t.mammal_0.2 <- list()
for (i in 1:length(data.mammal)){t.t.mammal_0.2[[i]] <- trainValTest(data.mammal[[i]], test = 0.2, only_presence = TRUE, seed = 4838)}

cut100 <- function(x){x <- x[1:100,]}
data.mammalp <- map(data.mammal, cut100)

t.t.mammal_0.2p <- list()
for (i in 1:length(data.mammal_pilot)){t.t.mammal_0.2p[[i]] <- trainValTest(data.mammal_pilot[[i]], test = 0.2, only_presence = TRUE, seed = 4838)}
...
rnd4k_mammal <- list()
for (i in 1:length(t.t.mammal_0.2)){rnd4k_mammal[[i]] <- randomFolds(t.t.mammal_0.2[[i]][[1]], k = 4, only_presence = TRUE, seed = 3667)}
...
folds.rnd4k_mammal.p <- map(t.t.mammal_0.2p, rnd4k)
...
maxnet <- function(x, y){train("Maxnet",
                                data = x[[1]],
                                folds = y)}

moedl_rndcv4k_mammal <- map2(t.t.mammal_0.2, rnd4k_mammal, maxnet)
model_rndcv5k_mammal <- map2(t.t.mammal_0.2, rnd5k_mammal, maxnet)  

moedl_rndcv4k_mammal.p <- future_map2(t.t.mammal_0.2p, folds.rnd4k_mammal.p, maxnet)

model_rndcv5k_mammal <- list()
for (i in 1:length(rnd5k_mammal)){model_rndcv5k_mammal[[i]] <- train("Maxnet", data =t.t.mammal_0.2[[i]][[1]], folds = rnd5k_mammal[[i]])}

# spatial block cross validation model

folds.block_mammal <- get.block(occ = data.mammal[[i]]@data[data@pa == 1, ], 
                                bg.coords = data.mammal[[i]]@data[data@pa == 0, ])

model_block_mammal <- train(method = "Maxnet", 
                            data = data, 
                            fc = "l", 
                            reg = 0.8, 
                            folds = folds.block_mammal)
...
test <- SpatialPointsDataFrame(data.mammal_pilot[[1]]@coords, data = as.data.frame(data.mammal_pilot[[1]]@pa), proj4string = crs(stackbio))
e_folds <- blockCV::envBlock(rasterLayer = stackbio, speciesData = test, k = 8, standardization = "standard", rasterBlock = TRUE, numLimit = 1)
...
```{r}
for (i in 77:93){
  model_rndcv4k_reptile[[i]] <- g[[i-76]]}

test_rndcv4k_mammal <- list()
for (i in 1:length(model_rndcv4k_mammal)){
  test_rndcv4k_mammal2[[i]] <- SDMtune::predict(model_rndcv4k_mammal[[i]],
                                               data = t.t.mammal_0.2[[i]][[2]],
                                               type = "cloglog")
}
test_xy_mammal <- list()
